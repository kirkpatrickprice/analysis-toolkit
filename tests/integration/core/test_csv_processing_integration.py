...
class TestCSVProcessingIntegration:
    ...
    def test_csv_processing_basic_file_reading_integration(
        self, tmp_path: Path, app_container: ApplicationContainer,
    ) -> None:
        ...
        service = app_container.core().csv_processor_service()
    ...
    def test_csv_processing_with_required_columns_integration(
        ...
        test_file = tmp_path / "test.csv"
        test_file.write_text("Name,Age,City,Salary\nJohn,25,NYC,50000\nJane,30,LA,60000\nBob,35,Chicago,55000")
    ...
    def test_csv_processing_encoding_detection_integration(self, tmp_path: Path) -> None:
        """Test CSV processing with automatic encoding detection."""
        # Create test CSV file with unicode characters
        test_file = tmp_path / "test_unicode.csv"
        unicode_content = "Name,Description\nCafé,A délicious café\nNaïve,A naïve approach"
    ...
    def test_csv_processing_with_custom_separator_integration(self, tmp_path: Path) -> None:
    ...
    def test_csv_processing_large_file_integration(self, tmp_path: Path) -> None:
        ...
        for i in range(EXPECTED_ROWS_LARGE):
            lines.append(f"Employee{i:04d},{20 + (i % 40)},Dept{i % 10},{40000 + (i * 10)}")
