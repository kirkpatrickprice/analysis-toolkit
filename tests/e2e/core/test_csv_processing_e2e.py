...
class TestCSVProcessingE2E:
    ...
    def test_csv_data_analysis_workflow_e2e(self, tmp_path: Path) -> None:
        ...
        test_file = tmp_path / "sales_data.csv"
        sales_data = [
        ...
        service = container.core().csv_processor_service()

        # Step 1: Load the data
        sales_data_df = service.read_csv_file(test_file)

        # Step 2: Validate data structure
        assert isinstance(sales_data_df, pd.DataFrame)
        assert len(sales_data_df) == SAMPLE_SALES_RECORDS
        expected_columns = ["Product", "Category", "Price", "Quantity", "Revenue", "Date"]
        assert list(sales_data_df.columns) == expected_columns

        # Step 3: Data analysis workflow
        # Group by category and calculate totals
        category_summary = sales_data_df.groupby("Category").agg({
            "Quantity": "sum",
            "Revenue": "sum",
        }).reset_index()

        # Verify analysis results
        electronics_data = category_summary[category_summary["Category"] == "Electronics"]
        furniture_data = category_summary[category_summary["Category"] == "Furniture"]

        assert len(electronics_data) == 1
        assert len(furniture_data) == 1

        # Verify calculations
        electronics_qty = electronics_data.iloc[0]["Quantity"]
        furniture_qty = furniture_data.iloc[0]["Quantity"]
    ...
    def test_csv_error_recovery_workflow_e2e(self, tmp_path: Path) -> None:
        ...
        files_to_process = [good_file, empty_file, malformed_file]
        ...
        for file_path in files_to_process:
            try:
                file_data_df = service.read_csv_file(file_path)
    ...
    def test_csv_service_integration_with_application_container_e2e(self) -> None:
        ...
        service1 = container.core().csv_processor_service()
        service2 = container.core().csv_processor_service()
