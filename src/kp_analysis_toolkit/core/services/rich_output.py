...
if TYPE_CHECKING:
    ...
    from kp_analysis_toolkit.models import ConfigValue, DisplayableValue, T
    from kp_analysis_toolkit.models.rich_config import RichOutputConfig


# Protocol for RichOutput service interface
class RichOutputProtocol(Protocol):
    """Protocol defining the RichOutput service interface."""
...
class RichOutputService:
    ...
    MOVE THE ENTIRE RichOutput CLASS CODE FROM utils/rich_output.py HERE.
    Rename the class from RichOutput to RichOutputService.
    Modify the constructor to accept RichOutputConfig instead of individual parameters.
    """

    def __init__(self, config: RichOutputConfig) -> None:
        """Initialize with configuration model instead of individual parameters."""
    ...
    def print(self, *args: RenderableType, **kwargs: Any) -> None:  # noqa: ANN401
        """Print directly to console (respects quiet mode)."""
        if not self.quiet:
            self.console.print(*args, **kwargs)

    def print_error(self, *args: RenderableType, **kwargs: Any) -> None:  # noqa: ANN401
    ...
    def format_value(self, value: DisplayableValue, max_length: int = 50) -> Text:
        """
    ...
    def configuration_table(
        ...
        original_dict: dict[str, ConfigValue] | None = None,
        *,
