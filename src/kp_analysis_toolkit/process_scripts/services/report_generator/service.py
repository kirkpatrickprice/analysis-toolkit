"""Default implementation of report generator service."""
...
class DefaultReportGenerator:
    ...
    def _process_search_configs(
        ...
        # Update statistics and process each search config
        self._update_search_statistics(yaml_config, stats)
        self._process_individual_searches(
            yaml_config.search_configs,
            searches_node,
            stats,
        )

        # Add searches node to tree
        tree_children = tree_node["children"]
        if isinstance(tree_children, list):
    ...
    def _update_search_statistics(
        ...
        """Update search statistics for OS family categorization."""
        if stats is None:
            return
    ...
    def _process_individual_searches(
        self,
        search_configs: dict[str, Any],
        searches_node: TreeNode,
        stats: StatsDict | None,
    ) -> None:
        """Process individual search configurations and add to searches node."""
        for search_name, search_config in search_configs.items():
            display_name: str = search_config.excel_sheet_name or search_name

            # Collect keywords for statistics
            self._collect_search_keywords(search_config, stats)
    ...
    def _collect_search_keywords(
        self,
        search_config: object,  # Individual search config from YAML
        stats: StatsDict | None,
    ) -> None:
        """Collect keywords from search config for statistics."""
        if (
            stats is not None
